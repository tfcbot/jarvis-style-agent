...
Get the person to a **working foundation**: a particle-orb frontend wired to an agent backend. The
brain (the backend) deploys to Vercel and is always on. The orb (the frontend) runs on the person's
machine and talks to the deployed brain. The person says "hello" and hears a spoken reply. That is the
deliverable, the clean base every powerful Jarvis-style system is built on.
...
2. `guide/01-frontend.md` — build the orb: Next.js app, the realtime voice wiring (token mint + the
   brain tool route), the orb render.
3. `guide/02-backend.md` — build the EVE brain: the agent, its instructions, the OpenAI-compatible shim.
4. `guide/05-deploy.md` — deploy the brain to Vercel (env vars, deploy-on-merge, the subdirectory gotcha). The orb runs on localhost.
...
- **Pin the pinned code.** Where a guide step says "write this verbatim," reproduce it exactly. These
  snippets (the OpenAI-compatible shim, the realtime token/tool routes, the orb shader, the brain
  adapters) are the parts that are easy to get subtly wrong. Assemble around them; do not rewrite
  them from memory.
...
A brain deployed at a URL, and an orb running on the person's machine that talks to it. The person
runs the orb with `bun run dev`, opens localhost, taps the status pill, says "hello," and hears a
live spoken reply — one they can talk over to interrupt — with data questions answered by their own
agent brain running on their own Vercel project with their own keys. Nothing
